...
//    ASSIGNMENT O6 -- Parser for tinyC
...
//  					Nakul Aggarwal (19CS10044)
	...
	#include <sstream>
	#include <algorithm>
	#include "ass6_19CS10044_19CS30053_translator.h"	// Include the header for the Translator File
	using namespace std;
	...
	extern vector<string> __string_literals__ ;	// stores all the string constants/literals seen in the source code
%}
...
__CT__: %empty 
		{ 															
			...
			if (__current_symbol__->_nested==NULL) {
				...
				newst->switchCurrentST() ;
			}	
			// Function symbol table already exists
			else 
			{
				__current_symbol__->_nested->switchCurrentST() ;					
				QuadArray::emit("func", __current_ST__->_name);
			}
		}
		;
...
primary_expression:	IDENTIFIER				
					...
					{ 
						...
						$$->_loc = SymbolTable::gentemp(new SymbolType("int"), str_stream.str()); // Generate a temporary for storing the integer constant (Example: t1)
					...
					{ 
						...
						QuadArray::emit("=@char", $$->_loc->_name, string($1)); // Emit the Three Address Code (Example: t1 = c)
						// important to have a different type of "equality" operator for character types
						// because the instructions for char and int types in asm language are different
						// (byte-level / word-level)
					} 
					| STRING_LITERAL
					{ 
						__string_literals__.push_back($1);	// push to the vector that stores all the string literals
						$$ = new E(); // create new expression E and store the value of the constant in a temporary
						...
						stringstream str_stream;
						str_stream << __string_literals__.size() - 1;
						QuadArray::emit("=@str", $$->_loc->_name, str_stream.str());
						// important to have a different type of "equality" operator for string types
						// because the instructions for strings and int types in asm language are different
						// (string literals are loaded from the data memory)
					}
...
postfix_expression:	primary_expression											
					...
					{ 
						...
						$$ = new A();	
						$$->_array = SymbolTable::gentemp($1->_type); // Generate a temporary to store the return value of the function
...
declarator:	pointer direct_declarator	
			{
				...
				while(t->_arrayType != NULL) {
					t = t->_arrayType;           
					// For multi-dimensional arrays, move till the base type of the array
				}
...
direct_declarator:	IDENTIFIER				
					...
					| direct_declarator OPENING_ROUND_BRACKET __CT__ parameter_type_list CLOSING_ROUND_BRACKET							
					{
						__current_ST__->_name = $1->_name;	// name of the ST should be the name of the function
						Symbol* ret = __current_ST__->lookup("return");		// look for the return variable of the function
						ret->updateType($1->_type);							// and update its type
						
						$1->_nested = __current_ST__;				// the nested ST for the function symbol
						__current_ST__->_parent = __global_ST__;	// function lies in the global scope
						__global_ST__->switchCurrentST();	// switch to global ST
						__current_symbol__ = $$;			// update the current symbol
						//$1->_size = 0;						// function definitions are of 0 width
						$1->_category = "function";			// category of the symbol
					...
					| direct_declarator OPENING_ROUND_BRACKET __CT__ CLOSING_ROUND_BRACKET							
					{ 
						__current_ST__->_name = $1->_name;	// name of the ST should be the name of the function
						Symbol* ret = __current_ST__->lookup("return");		// look for the return variable of the function
						ret->updateType($1->_type);							// and update its type
...
parameter_declaration: 		declaration_specifiers declarator	
							{ 
								$2->_category = "param";	// category of the symbol
							}
...
compound_statement:	OPENING_CURLY_BRACKET block_item_list_opt CLOSING_CURLY_BRACKET	
					{ 
						$$ = $2; 					// Equate the compound_statement to the block_item_list_opt 
...
iteration_statement: 	WHILE W OPENING_ROUND_BRACKET M expression CLOSING_ROUND_BRACKET M loop_body_statement			
						{ 
						
							// Single statement while loop
							
							$$ = new S();	// Create new statement
							convInt2Bool($5);	// Convert expression to boolean which has additional attributes truelist and falselist
							backpatch($8->_nextList, $4);	// After loop_body_statements, go back to expression again
							backpatch($5->_trueList, $7);	// If expression is true, goto M2 (just before statement body of while loop)
							$$->_nextList = $5->_falseList;   // When expression is false, move out of loop
							
							// Emit to prevent fallthrough
							string str = to_string($4);// Mark the start of evaluation of expression and put it in str	
							QuadArray::emit("goto", str);					// Emit the quad
							__current_loop__ = "";
						}
						| WHILE W OPENING_ROUND_BRACKET M expression CLOSING_ROUND_BRACKET OPENING_CURLY_BRACKET M block_item_list_opt CLOSING_CURLY_BRACKET   
						{	
							// Multiple statement while loop

							$$ = new S();	// Create new statement
							convInt2Bool($5);	// Convert expression to boolean which has additional attributes truelist and falselist
							backpatch($9->_nextList, $4);	// After loop_body_statements, go back to expression again
							backpatch($5->_trueList, $8);	// If expression is true, goto M2 (just before statement body of while loop)
							$$->_nextList = $5->_falseList;   // When expression is false, move out of loop
				   		...
				   		| DO D OPENING_CURLY_BRACKET M block_item_list_opt CLOSING_CURLY_BRACKET M WHILE OPENING_ROUND_BRACKET expression CLOSING_ROUND_BRACKET SEMICOLON      
						{
				   		...
				   		| FOR F OPENING_ROUND_BRACKET declaration M expression_statement M expression_opt N CLOSING_ROUND_BRACKET M loop_body_statement	
				   		{
				   			// Single Statement For Loop
				   		
				   			$$ = new S();		 	// Create new statement
							convInt2Bool($6);  // Convert expression to boolean which has additional attributes truelist and falselist
							backpatch($6->_trueList, $11);	// If expression is true, goto M3 (just before statement body of for loop)
							backpatch($9->_nextList, $5);	// After N (incrementing iterator), go back to M1 (the conditional expression)
							backpatch($12->_nextList, $7);	// After loop_body_statements, go back to expression again
							
							// Emit to prevent fallthrough
							string str = to_string($7);	// Mark the start of evaluation of expression and put it in str
							QuadArray::emit("goto", str);				// Emit the quad
							$$->_nextList = $6->_falseList;	// When expression is false, move out of loop
							__current_loop__ = "";
							// __current_ST__->_parent->switchCurrentST(); 
				   		}
				   		| FOR F OPENING_ROUND_BRACKET expression_statement M expression_statement M expression_opt N CLOSING_ROUND_BRACKET M loop_body_statement			
				   		{ 
				   			// Single Statement For Loop
				   		...
				   		| FOR F OPENING_ROUND_BRACKET declaration M expression_statement M expression_opt N CLOSING_ROUND_BRACKET M OPENING_CURLY_BRACKET block_item_list_opt CLOSING_CURLY_BRACKET     
						{
							...
							backpatch($6->_trueList, $11);	// If expression is true, goto M3 (just before statement body of for loop)
							backpatch($9->_nextList, $5);	// After N (incrementing iterator), go back to M1 (the conditional expression)
							backpatch($13->_nextList, $7);	// After loop_body_statements, go back to expression again
						...
						| FOR F OPENING_ROUND_BRACKET expression_statement M expression_statement M expression_opt N CLOSING_ROUND_BRACKET M OPENING_CURLY_BRACKET block_item_list_opt CLOSING_CURLY_BRACKET
						{	
							// Multiple Statement For Loop
...
function_definition:	declaration_specifiers declarator declaration_list_opt __CT__ OPENING_CURLY_BRACKET block_item_list_opt CLOSING_CURLY_BRACKET      
						{ 
							QuadArray::emit("funcend", __current_ST__->_name); // end of function definition reached
