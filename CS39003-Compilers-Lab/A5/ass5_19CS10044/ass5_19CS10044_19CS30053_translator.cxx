...
//    ASSIGNMENT O5 -- Translator File for tinyC
//    Semester O5 (Autumn 2021-22)
//    Group Members :	Hritaban Ghosh (19CS30053)
//  					Nakul Aggarwal (19CS10044)

#include "ass5_19CS10044_19CS30053_translator.h"
#include <iostream>
#include <exception>
#include <algorithm>
#include <iomanip>
...
Symbol::Symbol ( string name , string type , SymbolType * arrType , int symSize ) :
    _name(name), _type(new SymbolType(type, arrType, symSize)), _size(_type->size()), 
    _initVal(""), _nested(NULL), _offset(0), _isFunction(0), _retType(NULL) {  }    // constructor

void Symbol::print ( ) const {
    cout << "  | " << _name << string(max<int>(0, 23 - _name.size()), ' ') ;    // print name
    
    string type_string = _type->print() ;   // get the type of the symbol
    cout << " | " << type_string << string(max<int>(0, 20 - type_string.size()), ' ') ; // print type
    
    cout << "|  " << _initVal << string(max<int>(0, 12 - _initVal.size()), ' ') ;   // print initial value (will be an empty string "" if no initial value)
...
//   +----------------------------------------------------------------------------------------------------------------------+
//   |  TABLE NAME : F                               PARENT NAME : ST.Global                                                |
//   +----------------------------------------------------------------------------------------------------------------------+
//   | NAME                    | TYPE                | INIT-VALUE   | SIZE    | OFFSET      | NESTED                        |
//   | n                       | int                 |              | 4       | 0           | Null                          |
//   | thresh                  | float               |              | 8       | 4           | Null                          |
//   | A                       | ptr(float)          |              | 4       | 12          | Null                          |
//   | return                  | ptr(float)          |              | 4       | 16          | Null                          |
//   | F.FOR$0                 | block               |              | 0       | 20          | F.FOR$0                       |
//   | t#82                    | int                 |  0           | 4       | 20          | Null                          |
//   | t#83                    | char                |  'S'         | 1       | 24          | Null                          |
//   +----------------------------------------------------------------------------------------------------------------------+
...
void SymbolTable::print ( bool rec ) const { // (the entire routine is based on hit & trial of value of widths and 
    ...
    int width = 118 ;   // width of table (adjustable parameter)
...
void Quad::print ( ) const {    // method to print the quad (during lazy spitting)
    ...
    else if ( _op == "label" )  cout << _result << " :" << endl ;
